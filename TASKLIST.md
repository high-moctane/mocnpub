...
**作成日**: 2025-11-14
**最終更新**: 2025-12-05
**進捗**: Step 0〜4 完了！🎉 ブランチレス化で 3.14B keys/sec 達成！最適化はここで一区切り 🏁
...
- **`_Reduce512` ブランチレス化**（2025-12-05）🔥
  - ncu-ui ソースレベル分析で 324行目 `if (temp[4] > 0)` が 99.16% ダイバージェンスと特定
  - 「常に減算を計算 → マスクで選択」パターンでブランチレス化
  - `break` を削除し、必ず3回ループ（mask=0 で何もしない）
  - 結果：ダイバージェンス解消 ✅（パフォーマンスは横ばい）
  - **残りのダイバージェンス**：`_PointMult` のビット分岐（96.26%）→ アルゴリズム的に必要な分岐のため見送り
