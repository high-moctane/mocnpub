...
| # | タスク | 内容 | 状態 |
|---|--------|------|------|
| 1 | **CPU モードのコード削除** | `run_mine` 内の CPU 分岐（174-354行）を削除 | ⬚ |
| 2 | **`--gpu` フラグ削除** | 常に GPU モードに（フラグ不要に）| ⬚ |
| 3 | **`--threads` オプション削除** | CPU スレッド数指定は不要に | ⬚ |
| 4 | **不要な import 削除** | `mpsc`, `AtomicU64`, `AtomicUsize` 等を整理 | ⬚ |
| 5 | **ビルド & テスト** | `cargo build --release` & `cargo test` | ⬚ |
| 6 | **関数構造の整理** | `run_gpu_mining` を `run_mine` に統合 or リネーム | ⬚ |
| 7 | **CLI 説明文の更新** | `Mine` サブコマンドの説明を GPU 前提に | ⬚ |
| 8 | **最終確認** | 動作確認、コミット | ⬚ |

### 期待される効果

- **約200行のコード削減**
- **main.rs がシンプルに**（CPU/GPU の分岐がなくなる）
- **メンテナンスコスト減**
