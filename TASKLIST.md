...
| **`__ffs()` ビット反復最適化** | **逆効果（2025-12-26）**: `_PointMultByIndex` のループを `while + __ffs()` に変更。理論上はループ回数 24→~11 に削減されるが、-2.8%（5.78B → 5.64B）。原因：`__ffs()` オーバーヘッド、constant memory への飛び飛びアクセス、warp divergence 悪化 |
