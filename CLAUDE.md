...
| **`__ffs()` ビット反復最適化** | **逆効果と判明（2025-12-26）**: `_PointMultByIndex` のループを `for (bit=0; bit<24; bit++)` から `while (idx != 0) { bit = __ffs(idx)-1; idx &= idx-1; }` に変更。理論上はループ回数 24→~11 に削減されるが、実測で -2.8%（5.78B → 5.64B）。原因：(1) `__ffs()` のオーバーヘッド、(2) constant memory への飛び飛びアクセスでキャッシュ効率悪化、(3) while ループの終了タイミングがスレッドごとに異なり warp divergence 悪化 |
